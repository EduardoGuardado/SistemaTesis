...
<div class="row">
    ...
    <div class="col-sm-12 col-md-4 text-right">
        <a href="<?php echo base_url()?>index.php/ProfesionalControlador/insertar" class="btn btn-primary">Agregar</a>
    </div>
</div>
<br>
<div class="row">
    <div class="col-12" id="divTabla">
        <?php $this->load->view('profesional/tabla_profesional'); ?>
    </div>
</div>
...
<script>
    $(function(){
        $(document).on("click", "button[data-eliminar]", function (evt) {
            ...
            swal({
                title: "Eliminar",
                text: "¿Está seguro que desea eliminar el profesional?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            }).then((result) => {
                if (result) {                    
                    var data = {id: id};
                    $.post('<?php echo base_url()?>index.php/ProfesionalControlador/eliminar',data,function(response){
                        if(response == 'ok'){
        ...
        function buscar(){
            ...
            $.post('<?php echo base_url()?>index.php/ProfesionalControlador/buscar',{criterio:criterio}, function(response){
